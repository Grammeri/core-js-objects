...
function removeProperties(obj, keys) {
  ...
  // Удаляем указанные ключи из копии объекта
  keys.forEach((key) => {
    delete newObj[key];
  });
...
function compareObjects(obj1, obj2) {
  ...
  // Сначала проверяем, совпадает ли количество ключей
  if (keys1.length !== keys2.length) {
    return false;
  }

  // Затем проверяем, совпадают ли все ключи и их значения
  return keys1.every(
    (key) =>
...
function makeWord(lettersObject) {
  // Создаем массив достаточно большого размера, чтобы вместить все буквы.
  // Используем reduce для прохода по каждой букве и ее позициям.
  const wordArray = Object.entries(lettersObject).reduce(
    (acc, [letter, positions]) => {
      // Для каждой позиции добавляем букву в массив.
  ...
  // Преобразуем массив обратно в строку и возвращаем результат.
...
function fromJSON(proto, json) {
  ...
  // Установка прототипа для созданного объекта
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    // Сначала сравниваем страны
    if (a.country < b.country) return -1;
    if (a.country > b.country) return 1;

    // Если страны одинаковые, сравниваем города
    if (a.city < b.city) return -1;
    if (a.city > b.city) return 1;

    // Если и страны, и города одинаковые
    return 0;
  });
}
...
class CSSSelector {
  ...
  checkOrder(type) {
    ...
    if (this.order && this.order[this.order.length - 1] > orderMap[type]) {
      throw new Error(
  ...
  checkDuplicate(type) {
    ...
    if (this.order.includes(typeCode)) {
      throw new Error(
